            ...
            <div className="mx-auto grid w-full max-w-5xl overflow-hidden rounded-[24px] border border-stone-200 bg-white shadow-[0_20px_60px_rgba(28,25,23,0.06)] dark:border-stone-800 dark:bg-stone-900 dark:shadow-[0_20px_60px_rgba(0,0,0,0.3)] lg:grid-cols-[1.05fr_0.95fr]">
                <div className="relative hidden overflow-hidden bg-[#171513] p-8 text-white lg:flex lg:flex-col lg:justify-between">
                    ...
                    <div className="relative z-10">
                        ...
                        <h1 className="max-w-sm text-3xl font-semibold tracking-[-0.06em]">One small step toward honest conversations.</h1>
                    </div>
                    <p className="relative z-10 text-xs text-stone-300">Your verification code keeps your account secure.</p>
                </div>

                <div className="flex flex-col bg-[#f9f7f4] p-5 dark:bg-stone-900 sm:p-6 lg:p-8">
                    <div className="mb-6 sm:mb-8">
                        <div className="mb-3 inline-flex h-9 w-9 items-center justify-center rounded-xl bg-stone-900 text-xs font-semibold text-white dark:bg-stone-100 dark:text-stone-900">TF</div>
                        <p className="text-xs font-medium uppercase tracking-[0.18em] text-stone-500 dark:text-stone-400">Verify your email</p>
                        <h2 className="mt-1 text-2xl font-semibold tracking-[-0.06em] text-stone-900 dark:text-stone-100 sm:text-3xl">Enter your code</h2>
                    ...
                    <form onSubmit={submitCode} className="flex flex-1 flex-col justify-between" noValidate>
                        <div className="flex justify-between gap-2 sm:gap-3" aria-label="Six digit verification code">
                            {code.map((digit, index) => (
                                <Input
                                    ...
                                    onPaste={handlePaste}
                                    className="h-12 w-full rounded-xl border-stone-200 bg-white text-center text-lg font-semibold text-stone-900 shadow-none dark:border-stone-700 dark:bg-stone-950 dark:text-stone-100"
                                />
                            ))}
                        </div>

                        <Button type="submit" disabled={isSubmitting || code.some((digit) => !digit)} className="mt-6 h-10 w-full rounded-xl bg-stone-900 text-xs font-medium text-white shadow-[0_10px_25px_rgba(24,24,27,0.25)] hover:bg-stone-800 dark:bg-stone-100 dark:text-stone-900 dark:hover:bg-stone-200">
                            {isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" aria-hidden="true" />}
                            {isSubmitting ? 'Verifying...' : 'Verify email'}
                        </Button>

                        <div className="mt-6 border-t border-stone-200 pt-4 dark:border-stone-800">
                            <Link href="/sign-up" className="inline-flex items-center gap-2 text-xs font-medium text-stone-600 hover:text-stone-950 dark:text-stone-400 dark:hover:text-stone-100">
